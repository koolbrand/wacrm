...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Koolwaa CRM account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? 'our Koolwaa CRM account';
    const message = `Join ${accountName} on Koolwaa CRM using this link (valid for ${result?.expiresInDays} days): ${url}`;
